...
export function ResultPage() {
	...
	return (
		<PageChrome action={<ButtonLink variant="tertiary" to={`/games/${encodeURIComponent(result.gameId)}`}>退出到菜单</ButtonLink>}>
			<Surface tone={result.abandoned ? 'cream' : 'coral'} className={`${styles.resultHero} ${styles.fullBleed}`}>
				<div>
					<p className={styles.eyebrow}>{result.abandoned ? 'Abandoned record' : 'Run ended'} · {result.gameName}</p>
				...
				<div>
					...
					<div className={styles.pageActions}>
						...
						<ButtonLink variant="secondary" to={`/games/${encodeURIComponent(result.gameId)}/saves`}>选择存档</ButtonLink>
						<ButtonLink variant="tertiary" to={`/games/${encodeURIComponent(result.gameId)}`}>退出</ButtonLink>
					</div>
				</div>
			</Surface>
		</PageChrome>
