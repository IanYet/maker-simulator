	...
	return (
		<PageChrome>
			...
			<p className={styles.subtitle}>
				每个游戏都由外部脚本包驱动。开始一条时间线，在事件网络中构建状态，并从任意稳定检查点继续。
			</p>
			{state.status === 'loading' && (
				<StatusBanner tone="loading">正在读取游戏目录与脚本包…</StatusBanner>
			)}
			{state.status === 'error' && (
				<StatusBanner tone="error">无法读取游戏目录：{state.message}</StatusBanner>
			)}
			{state.status === 'ready' && state.games.length === 0 && (
				<StatusBanner tone="empty">目录中还没有游戏包。</StatusBanner>
			)}
			{state.status === 'ready' && state.games.length > 0 && (
				<section className={styles.gameGrid} aria-label="游戏列表">
					{state.games.map((game, index) => (
						...
						>
							<div>
								{game.coverLocation && (
									<img className={styles.cover} src={game.coverLocation} alt="" loading="lazy" />
								)}
