...
export class PackageLoader {
	...
	list(): Promise<LocatedGameCatalog> {
		this.#catalog ??= this.source.list().catch((error: unknown) => {
	...
	/** 按 catalog 的默认版本加载游戏。 */
	async loadDefault(id: string): Promise<LoadedGamePackage> {
		const catalog = await this.list()
		const version = catalog.defaultVersions[id]
		if (!version)
			throw new GamePackageLoadError('catalog', `Game “${id}” is not in the catalog`, {
				packageId: id,
			})
		return this.loadExact(id, version)
	}
