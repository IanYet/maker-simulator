import type { GamePackageSource, LocatedGameCatalog, LocatedGamePackage } from '../types/package'
 ...
 */
export class HttpPackageSource implements GamePackageSource {
	private readonly base: URL
	private readonly cache: RequestCache

	/** 使用宿主传入的绝对 base URL；同源规则以该资源源站为准。 */
	constructor(baseUrl: string, cache: RequestCache = 'default') {
		this.base = new URL(baseUrl)
		this.cache = cache
	}
	/** 读取并校验 catalog，同时检查包身份和默认版本引用。 */
	async list(): Promise<LocatedGameCatalog> {
		const location = new URL('games/catalog.json', this.base).href
		try {
