import type { Primitive, TurnPhase } from '../types/model'
...
/** 根据宿主传入的选项创建监控工厂，Runtime 不读取浏览器环境。 */
export function createMonitorFactory(
	setting: 'off' | 'basic' | 'verbose' = 'off',
): RuntimeMonitorFactory {
	return (runId) =>
		setting === 'off'
			? new NoopRuntimeMonitor()
			: new ConsoleRuntimeMonitor(setting === 'verbose', runId)
}
