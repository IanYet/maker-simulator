...
import type { Game, CommandResult, GameSnapshot } from '../types/game'
...
import { projectGameSnapshot } from './snapshot'
...
import type { RuntimeCommand } from './commands'
import { createRules } from './rules'
import { stateFromCheckpoint } from './snapshot'
...
function freezeSnapshot(snapshot: GameSnapshot): GameSnapshot {
 ...
 */
export class Runtime implements Game {
	...
	private readonly game: LoadedGamePackage
	...
	private constructor(
		...
		this.#state = stateFromCheckpoint(validatedProfile)
	...
	static async open(
		...
		try {
			runtime = new Runtime(game, profile, saves, monitor)
	...
	private dispatch(command: RuntimeCommand): Promise<CommandResult> {
		if (this.#disposed) {
		...
		return this.executeCommand(command).finally(() => {
			this.#busy = false
			if (this.#disposed) this.finishMonitor()
		})
	}

	/** 订阅稳定 GameSnapshot 发布；返回取消订阅函数。 */
	...
	startEvent(eventId: string): Promise<CommandResult> {
		return this.dispatch({ type: 'start-event', eventId })
	}
	/** 手动激活效果并稳定其 Reaction。 */
	activateEffect(effectId: string): Promise<CommandResult> {
		return this.dispatch({ type: 'activate-effect', effectId })
	}
	/** 执行当前单选节点选项。 */
	chooseSingle(eventInstanceId: string, nodeId: string, choiceId: string): Promise<CommandResult> {
		return this.dispatch({ type: 'choose-single', eventInstanceId, nodeId, choiceId })
	}
	/** 更新多选数量；旧节点或非法数量会被拒绝。 */
	setChoiceCount(
		...
		return this.dispatch({ type: 'set-multiple-choice', eventInstanceId, nodeId, choiceId, count })
	}
	/** 执行当前节点命令。 */
	executeNodeCommand(
		eventInstanceId: string,
		...
		return this.dispatch({ type: 'execute-node-command', eventInstanceId, nodeId, commandId })
	}
	/** 通过门禁提交本回合，随后进入下一回合。 */
	advanceTurn(): Promise<CommandResult> {
		return this.dispatch({ type: 'advance-turn' })
	}
	/** 放弃受同一命令锁保护，成功后产生 abandoned 检查点。 */
	abandon(): Promise<CommandResult> {
		return this.dispatch({ type: 'abandon' })
	}

	private async handleAbandon(): Promise<void> {
		if (this.currentRun().status !== 'active')
			throw new RuntimeFailure('invalid-phase', 'Only an active run can be abandoned')
	...
	/** 停止接收新命令与发布；已开始的保存完成后再结束监控。 */
	close(): void {
		if (this.#disposed) return
		this.#disposed = true
		this.#listeners.clear()
		if (!this.#busy) this.finishMonitor()
	}
	...
	private async executeCommand(command: RuntimeCommand): Promise<CommandResult> {
		...
		try {
			switch (command.type) {
				case 'start-event':
					await this.handleStartEvent(command.eventId)
					break
				case 'activate-effect':
					await this.handleActivateEffect(command.effectId)
					break
				case 'choose-single':
					await this.handleChooseSingle(command.eventInstanceId, command.nodeId, command.choiceId)
					break
				...
				case 'execute-node-command':
					await this.handleExecuteNodeCommand(
						command.eventInstanceId,
						command.nodeId,
						command.commandId,
					)
					break
				case 'abandon':
					await this.handleAbandon()
					break
				case 'advance-turn':
					await this.handleAdvanceTurn()
					break
			}
	...
	private async handleStartEvent(eventId: string): Promise<void> {
	 ...
	 * 失败时，激活状态和其它写入一起回滚。
	 */
	private async handleActivateEffect(effectId: string): Promise<void> {
		this.requireInputPhase()
	...
	private async handleChooseSingle(
		instanceId: string,
		nodeId: string,
		choiceId: string,
	): Promise<void> {
	...
	private async handleExecuteNodeCommand(
		instanceId: string,
		nodeId: string,
		commandId: string,
	): Promise<void> {
	...
	private async handleAdvanceTurn(): Promise<void> {
		...
		if (blockers.length > 0) {
			throw new RuntimeFailure('blocked', 'Required events must be completed before advancing')
		}
	...
	private createUnit(
		...
		const candidateReactive = cloneReactiveState(reactive)
		const unit: Unit = {
			...
			run,
			working,
			reactive: candidateReactive,
			rules: createRules(
				this.game,
				working,
				candidateReactive.graph,
				(call, duration, outcome, depth) => {
					if (this.#monitor.verbose)
						this.trace('rule-summary', call.key, duration, outcome, depth, unit, {
	...
	private profileView(unit: Unit, writable = false): ProfileRuntime | ActionProfileRuntime {
		if (!writable) return unit.rules.profile
		return createRuntimeView({
			...
			scope: 'profile',
			evaluateRule: (rule) => unit.rules.evaluate(rule),
	...
	private runView(unit: Unit, writable = false): RunRuntime | ActionRunRuntime {
		if (!writable) return unit.rules.run
		return createRuntimeView({
			config: this.game.config as GameConfig,
	...
	private turnView(unit: Unit, writable = false): TurnRuntime | ActionTurnRuntime {
		if (!writable) return unit.rules.turn
		return createRuntimeView({
			config: this.game.config as GameConfig,
	...
	private evaluateReaction(unit: Unit, definition: ReactionDefinition): Primitive {
		return unit.reactive.graph.read(this.reactionObserverId(definition.key), () => {
			...
			if ('source' in watch) {
				...
				value = unit.rules.evaluate(watch)
			}
	...
	private computeBlockers(): GameSnapshot['advanceTurnBlockers'] {
	...
	/** 将当前工作状态投影为 UI 使用的深度冻结 GameSnapshot。 */
	private selectSnapshot(
		state: RuntimeState,
		reactive: ReactiveRuntimeState,
		revision: number,
	): { snapshot: GameSnapshot; reactive: ReactiveRuntimeState } {
		const unit = this.createUnit('selector', state, reactive)
		try {
			const snapshot = projectGameSnapshot({
	...
	private traceRuleSummary(unit: Unit, outcome: 'ok' | 'error' | 'rollback' = 'ok'): void {
		if (unit.rules.stats.size === 0) return
		const slowest = [...unit.rules.stats.entries()].sort((a, b) => b[1].maxMs - a[1].maxMs)[0]
		const duration = [...unit.rules.stats.values()].reduce((sum, stat) => sum + stat.durationMs, 0)
		const widest = [...unit.rules.stats.entries()].sort(
			(a, b) =>
