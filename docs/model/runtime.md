...
| 顺序 | step | 说明 |
| --- | --- | --- |
| 1 | `turn_start` | 回合开始，结算启用效果的回合开始触发 |
| 2 | `combo_check` | 检查当前时机满足条件的效果组合 |
| 3 | `event_appear` | 遍历事件，筛选已解锁、已启用、满足出现条件的事件，再进行概率判定 |
...
2. 事件开始时，将 `currentNode` 设置为 `entryNode`。
3. 系统根据当前节点 `type` 执行对应逻辑。
4. 节点跳转时更新 `currentNode`。
5. 进入 `result` 节点且 `completeEvent=true` 时，写入事件结果并结束事件。
6. 事件完成后更新 `occurrences` 和 `completed`。
