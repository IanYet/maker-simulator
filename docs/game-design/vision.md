...
Config 中所有可变字段都是初始化默认值。State 没有保存对应值时使用 Config 默认值；Profile 中的写入覆盖 Config，RunData 中的写入覆盖当前 Profile。

字面值形式的 `ReactiveValue` 按 RunState、ProfileState、Config 默认值的顺序读取。配置了 Rule 的 `ReactiveValue` 始终使用 `calc` 的返回值作为有效值，该字段不能被直接写入；Action 通过修改 Rule 读取的基础事实改变结果。Rule 计算结果只由响应式系统缓存。
...
`id` 在其所属集合中必须唯一、非空，并且只由字母、数字、`-`、`_` 与 `.` 组成。`displayName` 与 `description` 是面向玩家的文本；程序逻辑不得用它们作引用。`tags` 可为空，单个对象内不得重复；标签只承担分类作用，不自动产生游戏效果。
...
interface CharacterConfig extends CommonConfig {
    attributeList: (NumberAttributeConfig | EnumAttributeConfig)[];
}

```

每个 character 的 `attributeList` 中，属性 `id` 必须唯一；角色 `id` 在 `characterList` 中必须唯一。属性 id 只要求在所属角色内唯一，因此不同角色可以各自拥有同名的 `health` 属性。
...
效果获得与激活的直接状态值保存在 RunState 的 `objectValueMap`，发生回合保存在 `effectRuntimeMap`。所有 EffectConfig 的 Reaction 在创建、载入、分支或截断恢复 RunData 时注册，无需等待 EffectRuntime 记录出现；注册时只建立基准值。依赖图、已解析值和上次值属于引擎内部运行时缓存，可由 State 重新构建。
...
`nodeList` 中的节点 id 在同一事件内必须唯一，`entryNodeId` 必须指向其中一个节点。
...
单选节点中，玩家选择一个 `SingleChoice` 后立即执行其 `action`。多选节点中，增减 `MultipleChoice` 数量只更新 TurnData 中的临时选择，不执行 Action；选择结果至少包含 `choiceId`、`value` 与 `count`。

多选节点通过 `commandList` 提供购买、确认、取消或退出等操作。Command Action 一次性读取完整选择结果；事务失败时不提交任何选择效果。退出 Command 可以清空临时选择并离开节点。`maxCount` 存在时限制对应 choice 的最大选择数量，省略时不设置引擎级单项上限。`required` 为 `true` 时，节点尚未处理完成则不能进入下一回合；`chance` 表示该 TextNode 出现的机会。
...
`nodeList` 列出该检查节点可指向的节点；其中所有 id 都必须指向本事件内存在的节点。计算下一个节点时，使用候选 TextNode 已解析的 `chance`。检查规则和目标节点选择的具体配置在 `check` 动作中定义。
...
存档容器与脚本访问视图分离。Rule 与 Action 参数中的 `profile`、`runData`、`turnData` 分别是当前 ProfileState、RunState 与 TurnState 的 Proxy；id、历史索引、时间戳、存档版本等容器元数据由引擎维护，脚本不能直接改写。
...
type ConfigObjectRef = string;

interface StateValues {
    /** Config 对象字段的直接状态值。 */
    objectValueMap: Record<ConfigObjectRef, Record<string, JsonValue>>;

    /** characterId -> attributeId -> 当前数值。 */
    attributeValueMap: Record<string, Record<string, number>>;
...
type ProfileState = StateValues;

interface RunState extends StateValues {
    /** effectId -> Effect 的附加运行时信息。 */
    effectRuntimeMap: Record<string, EffectRuntime>;
    eventInstanceMap: Record<string, EventInstance>;
}
```

`ConfigObjectRef` 是包含所属关系的规范引用。角色、Effect 和 Event 等顶层对象可分别使用 `character/{characterId}`、`effect/{effectId}` 和 `event/{eventId}`；局部对象必须继续包含 owner 路径，例如 `event/{eventId}/node/{nodeId}/choice/{choiceId}`。存档不能只用局部 node id 或 choice id 定位对象。

`objectValueMap` 只保存 Action 实际写入的状态字段。Effect 的字面值 `acquired` 与 `actived` 保存在 RunState 的 `effect/{effectId}` 对应记录中，CommonConfig 的字面值 `unlocked` 与 `enabled` 使用所属对象的规范引用。没有 State 值时回退到 Config 默认值；同一字段同时存在于 ProfileState 与 RunState 时，当前 RunData 中的值优先。

字段值包含 Config 对象或 Config 对象列表时，State 保存对应的 `ConfigObjectRef` 或引用列表，读取时由引擎解析成目标对象。State 中不嵌入 Config 对象本身。

配置了 Rule 的 ReactiveValue 字段不能写入 `objectValueMap`，其中的同名值也不会参与有效值计算。Action 必须改写该 Rule 依赖的 ProfileState、RunState 或 TurnState 基础事实。

`attributeValueMap` 使用 character id 与 attribute id 两级定位属性。AttributeConfig 的 `value` 只能写入 `attributeValueMap`，不能同时写入 `objectValueMap`；缺少属性值时使用对应 AttributeConfig 的 `value`。`valueMap` 保存剧本需要的额外事实，key 必须由内容包自行保证稳定且唯一。

每个 StateValues 的三个 map 都必须存在，没有数据时使用空对象；RunState 还必须包含 `effectRuntimeMap` 与 `eventInstanceMap`。State 保存相对低层数据的写入结果，无需复制未被修改的 Config 默认值。

Rule 派生字段的有效值不写入任何 map。Rule 读取的基础事实保存在对应 State 中，载入后重新计算有效值。Config 对象、函数、Proxy、缓存以及不能 JSON 序列化的对象都不能写入 `StateValues`。

新游戏与 restart 初始化 RunState 时，初始有效值为 `true` 的 `acquired` 或 `actived` 使用当前的 `turnNumber = 0` 初始化对应 EffectRuntime，表示该 Effect 在本 Run 开始时已经获得或激活。Reaction 注册仍只建立基准值，因此不会把初始 `true` 当作状态转换执行一次性 Action；需要在开局执行的 Action 应通过观察 `initializing` 到 `turn_start` 的 Reaction 表达。

初始化之后任一字段从 `false` 变为 `true` 时，引擎写入当前回合数；`actived` 再次进入 `true` 时更新 `activedTurn`。载入、branch 与截断恢复保留 snapshot 中的 EffectRuntime，也不会重复执行已经发生过的生命周期 Action。

## 事件状态

每次成功执行 `start_event` 都创建一个 EventInstance，并存入当前 RunState 的 `eventInstanceMap`：
...
`instanceId` 在所属 RunData 内唯一，`eventId` 引用 EventConfig。每个实例独立记录当前节点和实际访问路径；进入节点时把 node id 追加到 `nodePath`。实例完成或放弃时写入 `status` 与 `endedTurn`，并保留在 `eventInstanceMap` 中，Rule 可以据此结合 Config 中的 event tag 计算完成次数等派生值。
...
interface TurnState {
    ...
    turnNumber: number;
    phase: TurnPhase;
    /** key 由 eventInstanceId 与 nodeId 共同确定。 */
...
多选 TextNode 修改 `selectionMap`。`count` 必须是正整数；对应 MultipleChoice 配置了 `maxCount` 时，还不能超过其已解析值。同一节点的同一个 choice 只保留一条记录。Command Action 成功提交、退出节点或事件结束时清除对应记录。新回合初始化时清理上一回合未继续使用的临时选择。
...
- EventInstance 与 EffectRuntime 始终保存在 RunState；属性值属于写入它们的 ProfileState 或 RunState，写入层级决定其跨局生命周期。
...
ProfileState、RunState 与 TurnState 的底层数据保持为可 JSON 序列化的普通对象。引擎运行时在它们外层创建 Proxy：Rule 使用只读 Proxy，Action 使用事务内的可写 Proxy。Proxy 只负责追踪访问和写入，不改变存档格式。Config 直接以只读对象注入；由于一局游戏中 Config 不可变，对它的读取不登记依赖。
