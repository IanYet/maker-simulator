...
pin 只保护自动保留策略。玩家确认后可以手动删除任意 TurnData、整条 RunData 或整个 StoredProfile，显式删除不会因目标或其后代包含已 pin 检查点而被拒绝。删除来源检查点或来源时间线不级联删除 branch/restart 后代；后代保留完整 snapshot，`RunOrigin.source` 降级为不可解析的历史引用。
...
StoredProfile、RunData 与 TurnData 只保存可 JSON 序列化的稳定检查点、时间线元数据和恢复游标；游戏脚本写入的结局字段属于 snapshot 中的普通状态事实。存档不额外保存当前工作 State，也不复制 Config 中的展示文本、tag、节点图、Rule/Action 函数或 Reaction 配置。依赖图、Rule 缓存、Effect 生命周期观察值、Reaction baseline、Proxy、事务 draft、待处理的终局请求和执行队列也不写入存档，载入后由引擎重新构建。
...
引擎接收已经由包加载器校验并链接完成的 `LoadedGamePackage`，不会在局级运行时中 fetch JSON 或 import JavaScript。Config、ActionRegistry 与 RuleRegistry 属于包级只读数据；每个 RunData 只创建与当前 State 绑定的执行器、Proxy、依赖图、持续 observer、Reaction baseline 和 FIFO。外部入口与包级 linking 见[外部游戏包与加载](./game-package.md)。
...
3. 按新游戏相同顺序创建事务、Proxy、Rule/Action executor、依赖图与 observer baseline。
...
初始化的固定先后关系是：初始生命周期事实先于 `initial` snapshot，持久化的 `initial` 先于 Runtime baseline，registry executor 先于 baseline，baseline 先于首次 `turn_start`。初始结构构造失败不会创建存档；baseline 或首次回合处理失败只留下可独立恢复的 `initial`。
...
Config 与 State 使用相同的 object 字段名：顶层都是 `characters`、`effects` 与 `events`；Character 下都是 `attributes`；Event 下都是 `nodes`，TextNode 下都是 `choicesValue`。新 Run 的 RunState 会物化 ProfileState 覆盖 Config 后的基础字段；不完整或结构过期的存档在加载校验时直接拒绝。
...
interface TurnState extends GameState {
    turnNumber: number;
    phase: 'initializing' | 'turn_start' | 'event_handle' | 'turn_end';
}
```

State 中的每一项通过 object key 与同层 Config 对象对应，value 的 `id` 必须等于 key。新 Run 的 RunState 会包含 ProfileState 覆盖 Config 后的基础值所对应的对象和 `xxxValue` 字段；未发生其它写入的字段仍可省略。三个顶层 object 始终存在，没有数据时使用空对象。
...
Rule 字段不能保存同名 State 值，Action 必须改写对应的 `xxxValue` 基础字段。Rule 派生字段的有效值不写入 State；载入、branch 和截断恢复时恢复基础 State，再重新计算有效值。Config 中的 Rule、Action、Proxy 以及不能 JSON 序列化的对象都不能写入 `GameState`。

新游戏与 restart 初始化 RunState 时，ProfileState 覆盖 Config 后为 `true` 的 `acquiredValue` 或 `activedValue` 使用当前的 `turnNumber = 0` 初始化对应 EffectState 的回合字段，表示该 Effect 在本 Run 开始时已经获得或激活。Reaction 注册仍只建立基准值，因此不会把初始有效值 `true` 当作状态转换执行一次性 Action；需要在开局执行的 Action 应通过观察 `initializing` 到 `turn_start` 的 Reaction 表达。
...
`turnId` 在所属 RunData 内唯一，`createdAt` 是该检查点的提交时间。`snapshot` 在创建后保持只读，`pinned` 是可以单独修改、只影响自动清理的存档元数据；同一 RunData 中检查点的先后关系由 `turnOrder` 表达。
...
引擎使用依赖图连接 State、Rule 计算节点和持续 observer。Rule 执行期间读取的 State 字段、对象成员、集合 key 和嵌套 Rule 都会成为动态依赖；State 写入只使反向可达的计算节点失效。Reaction watch 与 Effect 的 `acquired`/`actived` 生命周期是持续 observer，只有依赖失效后才主动重算。
...
2. 按本批 State 写入路径沿反向依赖边标记 Rule、Effect 生命周期与 Reaction observer 失效；
3. 重算 dirty Effect 生命周期 observer，写入新出现的 `acquiredTurn`/`activedTurn` 并继续传播失效；
4. 按 canonical ordinal 重算 dirty Reaction observer，与各自 baseline 比较；
...
因此，完成一个黑暗事件的 Action 只需写入事件完成记录。若某个 Effect 的 `actived` Rule 读取该集合，集合依赖会使对应 Rule 和生命周期 observer 失效并重算，无需 Action 发布专用 signal 或重复 Rule 中的条件。
...
Config 直接以只读对象放入 context；Config 一局内不可变，因此读取不登记依赖。State 依赖以视图层级和 Record 路径定位。ProfileState 写入影响 Profile/Run/Turn 三层视图，RunState 写入影响 Run/Turn 视图，TurnState 写入只影响 Turn 视图；写入路径的祖先同时失效，以覆盖 `ownKeys`、对象成员和集合迭代。

每个 `[ruleKey, args]` 对应一个计算节点。节点保存当前依赖、反向依赖、dirty 状态和可复用结果；未失效的基础类型结果直接命中缓存，失效后在被 selector、Effect 生命周期或 Reaction 读取时惰性重算。对象结果可能包含绑定当前 Immer draft 的 State Proxy，因此只替换依赖而不跨读取缓存。同步调用栈检测递归，计算异常不保存本次结果或动态依赖。

每次成功重算都会用本次实际读取集合替换旧依赖，从而支持条件分支。嵌套 `context.rule` 调用形成计算节点之间的边；底层 State 路径失效会沿这些边传播到所有持续 observer。UI selector 仍按 read model 结构投影 snapshot，但未失效的 Rule 直接复用缓存。

Rule 与 Action 都通过统一执行器获得 context。处理单元创建一次 State/PRNG draft；嵌套 `context.action` 和 Reaction Action 复用该 draft，`context.rule` 在 Action 内读取包含当前未提交写入的视图。终局请求不携带结局数据，同一处理单元中的重复请求合并为一个标记。

Reaction 在 Runtime 建立或首次进入作用域时把 `watch` 注册为持续 observer，并保存当前结果作为 baseline。State 写入后只重算依赖失效的 observer，使用 `Object.is` 比较新旧结果，再根据 `from`、`to` 决定是否将 Action 放入执行队列。用于 Reaction 的 Rule 必须返回 Primitive。

所有 EventConfig 与 EffectConfig Reaction 在 Runtime 构造时注册一次；TextNode Reaction 在 EventInstance 进入节点时精确注册，跳转或结束时精确注销。载入、分支或截断恢复时，引擎为每个 active EventInstance 的当前 TextNode 恢复 observer。注册和恢复只建立 baseline，不执行 Reaction Action。`terminal` 或 `abandoned` 提交后注销所属 RunData 的全部持续 observer。
...
同一批失效使多个 Reaction 同时匹配时，按 ordinal 加入 FIFO；Reaction Action 引发的新匹配追加到队尾。存在终局请求时，引擎先处理完同一因果链，再提交终局；请求进入待处理状态后不再接受新的玩家操作或阶段推进。单个处理单元最多执行 512 个 Action frame、4096 次 Rule 重算和 128 次自动 CheckNode 跳转；超过限制时回滚并输出调用链。

引擎统计每个 Rule 的重算次数、执行耗时、依赖数量与反向扇出，用于定位频繁重算、计算缓慢或影响范围过大的 Rule。一次 State 写入的响应式成本由受影响的反向可达子图决定，不与全部 Reaction 数量直接绑定。

依赖图、计算缓存、Effect 生命周期观察值和 Reaction baseline 都是可重建的运行时数据，不写入存档。从 Profile、RunData 或 TurnData 恢复游戏时，引擎重新创建 Proxy 和依赖图，计算全部持续 observer 的 baseline，此时不执行 Reaction Action。
