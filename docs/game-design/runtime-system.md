...
| RunData | 一条独立的局内时间线，保存本局状态、事件实例和回合检查点 | 从一局开始到由 Action 结束、放弃或重开 |
| TurnData | 一条时间线在稳定边界上的可恢复检查点 | 在 RunData 初始化、回合提交、终局或废弃提交时创建 |

Config 是只读内容源。Profile 持有 ProfileState 与 RunData 集合，RunData 持有 RunState、TurnState 和检查点索引，TurnData 通过 snapshot 保存各层 State；这些 Data 对象是持久化容器，不作为脚本 Context 传入。Rule/Action 只接收按层级解析后的 State 视图，并通过稳定 id 引用 Config 中的角色、属性、效果、事件和节点；调用描述与 Reaction 保留在 Config，JavaScript 实现保留在 LoadedGamePackage 的 registry。
...
3. 选择继续游戏时，引擎恢复 `Profile.current` 指向的检查点。`initial` 与 `turn_end` 从下一回合开始；`terminal` 或 `abandoned` 只恢复结果/历史界面。浏览历史不会修改恢复游标。
...
7. 玩家退出游戏界面或在游戏内打开存档浏览器时，引擎丢弃本回合初始化以来的全部未提交工作状态；`Profile.current` 保持指向进入本回合前的 `initial` 或上一 `turn_end`。
...
- 选择删除时，引擎从原 RunData 的 `turnOrder` 与 `turnDatas` 中删除所选检查点之后的全部 TurnData，以所选 snapshot 重建包括结局字段在内的工作状态，并把 `currentTurnId` 与 `Profile.current` 指向所选检查点。若原 RunData 已结束，同时把 status 恢复为 `active` 并清除 `endedAt`，后续回合继续写入该 RunData。
...
Profile、RunData 与 TurnData 只保存可 JSON 序列化的状态事实、Action 执行结果和 RunData PRNG 状态；游戏脚本写入的结局字段属于普通状态事实。存档不复制 Config 中的展示文本、tag、节点图、Rule/Action 函数或 Reaction 配置。Rule 计算结果、依赖图、计算缓存、Reaction 基准值、Proxy、事务 draft、待处理的终局请求和执行队列也不写入存档，载入后由引擎重新构建。
...
1. 根据 Config 创建空的稀疏 ProfileState、RunState 与 TurnState 顶层集合，生成 id、时间、PRNG seed，并设置 `turnNumber = 0`、`phase = 'initializing'`。Config 默认值通过运行时回退读取，不复制整棵 Config。
2. 创建事务管理器、PRNG draft、Config 与各层 State 的合并 Proxy。
3. 把纯 RuleRegistry 包装成当前 Run 的 Rule executor，再把 ActionRegistry 包装成事务 Action executor。`context.rule` 与 `context.action` 是绑定函数集合，registry 本身保持只读。
4. 编译 ReactiveValue 计算节点与依赖图；此时可以计算 Rule，不执行 Action。
5. 物化必须保存的回合 `0` 生命周期事实，例如初始 `acquired`、`actived` Effect 的发生回合。
6. 在 provisional 容器中校验初始 State 并构造 `initial` TurnData，此时尚未加入 Profile 或写入存档。
7. 按确定顺序注册 EffectConfig、EventConfig 与当前节点 Reaction，只计算基准值；任一 watch Rule 失败就丢弃 provisional 容器。
8. baseline 成功后，原子把带 `initial` 的 RunData 加入 Profile 并持久化；这是新局构造的成功边界。
9. 引擎以 `initial` 为回滚边界，把 `turnNumber` 增加到 `1`，进入 `turn_start`，运行到 `event_handle` 稳定输入点后发布首个 UI snapshot。首回合脚本失败时保留完整 `initial`，报告错误并允许修复包后重试，不留下半提交的回合状态。

### 载入、branch 与截断恢复

1. 根据 `Profile.configId` 与 `configVersion` 取得精确游戏包，完成迁移并校验所选 snapshot。
...
初始化的固定先后关系是：registry executor 先于计算节点，初始生命周期事实先于 `initial` snapshot，Reaction baseline 先于挂入 Profile，持久化的 `initial` 先于首次 `turn_start`。构造/baseline 失败不会创建 Profile/RunData；首次回合处理失败只留下可独立恢复的 `initial`。
...
`EffectConfig.manuallyActivatable` 是 Config 中的静态能力声明，不进入 EffectState。它为 `true` 且 Effect 已获得、尚未激活时，`activate-effect` RuntimeCommand 才能在 `event_handle` 阶段被接受。Runtime 在同一处理单元中校验 `visible`、`unlocked`、`enabled`、`acquired` 与 `actived`，通过可写 RunState 视图将 `actived` 设为 `true`，然后运行正常的 Reaction 稳定流程。手动激活的 `actived` 必须是字面 ReactiveValue；Rule 派生字段由其依赖事实驱动，不能由该命令直接赋值。
...
`profileId` 是存档 id，`label` 是玩家可修改的可选显示名。`stateVersion` 确定存档结构的迁移逻辑，`configId` 与 `configVersion` 确定该存档所需的内容包及内容迁移逻辑。`state` 是基于恢复游标创建的局外工作状态；`runDatas` 保存该 Profile 的所有时间线；`current` 指向最后提交或由玩家选择继续的检查点。该检查点为 `terminal` 或 `abandoned` 时只能恢复结果界面。

`createdAt` 在新游戏时写入，`updatedAt` 在修改 label、提交回合、结束 RunData、创建分支、修改 pin 或迁移存档后更新。`current` 必须指向 `runDatas` 中存在的 RunData 与 TurnData。查看历史记录、展开分支或在界面中选中检查点不会修改 `current`。
...
恢复 `Profile.current` 时，引擎用其 `snapshot.profileState`、`snapshot.runState`、`snapshot.turnState` 与 `snapshot.randomState` 创建当前时间线的工作副本。`terminal` snapshot 中的结局仍按普通 RunState 字段读取，`terminal` 与 `abandoned` 不得进入状态机。
...
- 一个运行时处理单元中的 State、PRNG、EventInstance 派生写入与终局请求，检查点和 RunData 元数据，以及存档文件分别以原子方式提交，任何一层失败都不能留下部分写入。
...
7. 队列稳定后检查待处理终局请求，存在请求时原子提交 `terminal` 与 `ended` 生命周期，否则提交普通处理结果或对应检查点；
8. 任一脚本、校验、循环上限或该单元的持久化失败时丢弃整个处理单元，UI 继续使用最近的稳定 snapshot。
