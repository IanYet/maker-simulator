...
StoredProfile、RunData 与 TurnData 只保存可 JSON 序列化的稳定检查点、时间线元数据和恢复游标；游戏脚本写入的结局字段属于 snapshot 中的普通状态事实。存档不额外保存当前工作 State，也不复制 Config 中的展示文本、tag、节点图、Rule/Action 函数或 Reaction 配置。Rule 计算结果、Reaction 基准值、Proxy、事务 draft、待处理的终局请求和执行队列也不写入存档，载入后由引擎重新构建。
...
引擎接收已经由包加载器校验并链接完成的 `LoadedGamePackage`，不会在局级运行时中 fetch JSON 或 import JavaScript。Config、ActionRegistry 与 RuleRegistry 属于包级只读数据；每个 RunData 只创建与当前 State 绑定的执行器、Proxy、Reaction baseline 和 FIFO。外部入口与包级 linking 见[外部游戏包与加载](./game-package.md)。
...
5. 按确定顺序收集 EffectConfig、EventConfig 与当前节点 Reaction，只计算基准值；baseline 失败时关闭 Runtime，已保存的 `initial` 保持不变。
6. Rule executor 已可供 baseline 和 UI 读取使用；每次调用都直接计算，不执行 Action。
...
3. 按新游戏相同顺序创建事务、Proxy、Rule/Action executor 与 Reaction baseline。
...
引擎采用确定性的 eager scan。Rule 在每次读取时直接执行，不缓存结果，也不收集 State 依赖；Reaction baseline 是响应式系统唯一长期保留的派生数据。
...
2. 按 canonical ordinal 重新收集当前全部 EffectConfig、EventConfig 与 active TextNode Reaction；
3. 逐个计算全部 Reaction watch，并与各自 baseline 比较；
4. 新出现的 Reaction 只建立 baseline，已经离开作用域的 Reaction 删除 baseline；
...
root 操作完成后执行一次全量扫描；每个 Reaction Action 完成事件派生写入和 Effect 生命周期同步后再次全量扫描。已经入队但执行前离开作用域的 Reaction 会被跳过。引擎为自动 Action 链维护队列并检测同步 Rule 递归或超出执行上限的情况。待处理的终局请求不会跳过已由本次状态变化触发的 Reaction 队列。

因此，完成一个黑暗事件的 Action 只需写入事件完成记录。若某个 Effect 的 `actived.rule` 读取该集合，后续 Reaction 扫描和 UI 读取都会得到更新后的有效值，无需 Action 发布专用 signal 或重复 Rule 中的条件。
...
Config 直接以只读对象放入 context；Config、State 与运行时视图本身通过 object key 提供稳定定位，引擎使用所属路径和 key 进行合并与查询。

Rule executor 每次调用都创建当前处理单元的只读 context 并直接执行实现。同步调用栈使用 `[ruleKey, args]` 的稳定标识检测递归；异常不产生结果。UI selector 在稳定处理单元之外构建 snapshot 时也会重新执行所读取的 Rule，因此 Rule 的纯计算契约适用于所有调用位置。
...
Reaction 在 Runtime 建立或首次进入作用域时保存 `watch` 的当前结果作为基准。每次扫描都计算全部当前 watch，使用 `Object.is` 比较新旧结果，再根据 `from`、`to` 决定是否将 Action 放入执行队列。用于 Reaction 的 Rule 必须返回 Primitive。

Reaction 的扫描范围由声明位置决定：所有 EventConfig 与 EffectConfig 的 Reaction 始终参与；TextNode 的 Reaction 只在 EventInstance 位于该 active 节点时参与。载入、分支或截断恢复时，引擎还会为每个 active EventInstance 的当前 TextNode 建立 baseline。恢复过程只建立基准值，不执行 Reaction Action。`terminal` 或 `abandoned` 提交后清除所属 RunData 的全部 baseline。
...
一次扫描使多个 Reaction 同时匹配时，按 ordinal 加入 FIFO；Reaction Action 引发的新匹配追加到队尾。存在终局请求时，引擎先处理完同一因果链，再提交终局；请求进入待处理状态后不再接受新的玩家操作或阶段推进。单个处理单元最多执行 512 个 Action frame、4096 次 Rule 和 128 次自动 CheckNode 跳转；超过限制时回滚并输出调用链。

引擎统计每个 Rule 的执行次数与执行耗时，用于定位频繁执行或计算缓慢的 Rule。一次扫描的成本与当前 Reaction 数量及其 Rule 调用链成正比；游戏包应保持 watch 和 Rule 轻量。只有实际性能分析表明全量扫描成为瓶颈时，才考虑引入依赖跟踪；它不属于当前脚本契约。

Reaction 基准值是可重建的运行时数据，不写入存档。从 Profile、RunData 或 TurnData 恢复游戏时，引擎重新创建 Proxy，并按 canonical 顺序计算当前全部 Reaction baseline，此时不执行 Reaction Action。
