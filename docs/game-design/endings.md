...
- `abandoned` 表示玩家放弃本局。废弃不要求游戏脚本写入结局；玩家之后可以从该记录创建 restart RunData。
...
若首次 `endRun()` 请求的调用链关联到一个 EventInstance 的当前 TextNode，引擎在 terminal TurnData 的 `endingEventInstanceId` 中记录该实例 id，并可把 snapshot 中的节点投影为可选 `endingEvent`，供结果页保留结局叙事。重复请求不替换首次来源。配置级 Reaction 或阶段 Action 请求终局时不一定存在关联节点；此时 ended 生命周期与 terminal snapshot 仍然完整有效，UI 显示通用终局信息。
