...
- 一个外部游戏包对应游戏列表中的一个游戏。
- UI 只读取引擎提供的不可变视图，并调用宿主命令；UI 不直接查找或执行 JavaScript Action 函数。
...
“继续游戏”的目标由应用级存档索引记录的最近使用 Profile 决定。不能只使用 `Profile.updatedAt` 推断目标，因为 pin、迁移或整理存档也可能修改更新时间。
...
不兼容、迁移失败或损坏的 Profile 仍保留在列表中，但禁止继续，并显示可定位的原因。浏览其他有效 Profile 不受单个损坏存档影响。
...
UI 不直接写入 phase，也不通过某个脚本 Action 模拟阶段切换。
...
| 退出并放弃 | 放弃整条当前 Run，创建 `abandoned` 检查点 | 隐藏或禁用 | 必须二次确认 |
...
| 服务 | 职责 |
| --- | --- |
| PackageCatalog | 发现游戏包、读取轻量元信息并报告加载错误 |
| SaveRepository | 按 Config id 列出 Profile，负责原子读写、迁移和兼容检查 |
| GameSession | 加载当前 Config 和 Profile，执行宿主命令并暴露 busy 状态 |
| SessionView | 向 UI 提供不可变、已解析的属性、Effect、事件、节点、门禁和生命周期状态 |

React 组件不得持有运行时 Proxy，不得在 render 中执行 Rule，也不得直接调用 Action 的 `exec`。React StrictMode 下的重复 render 不得产生引擎写入、Action 调用或 PRNG 推进。

`GameSession`、`SessionView`、应用按钮方法以及存档树的 `SaveCommand`/`SaveBrowserController` 公共类型统一定义在 [runtime.ts](../../src/types/runtime.ts)。分支、截断和 pin 必须通过该 controller 调用存档服务，组件不直接改写 Profile。
...
| 内容版本迁移 | game package 尚未定义 migration module API | MVP 只加载 exact version；迁移接口单独设计后再允许升级 | 同意 |
