...
`put` 必须在同一个读写事务中读取现有记录、比较 `storageRevision`、写入递增后的对象并等待 `tx.done`，之后才返回已保存对象。版本不匹配时抛出冲突，不能覆盖另一个页面的新记录。运行时 draft、Proxy、LoadedGamePackage 和函数不得传入 repository。写入前以 `structuredClone` 产生纯数据副本并执行结构校验；精确 Config 领域校验在应用层或 Runtime 进入 Repository 前完成。列表查询逐条隔离坏记录，不能因单条损坏数据阻断其它存档。

`validateStoredProfile(unknown)` 负责 schema、游标、检查点集合和 RunData 生命周期等不依赖游戏内容的约束。`validateProfileAgainstConfig(profile, config)` 负责 Config 身份、全部 State key/id、EventInstance、选择、Effect 绑定、终局引用、回合单调性和 State 层级约束。存档列表可以只执行第一层；标记可继续状态以及任何继续、分支、截断、restart、Runtime 打开或候选检查点写入前必须加载精确包并执行第二层。领域错误携带 JSON Pointer，原始记录保持不变。
...
- 当前 Reaction 定义、baseline 和 FIFO；
...
提交顺序：执行 root 操作 → 验证 Action frame → 生成事件派生写入 → 全量扫描当前 Reaction watch → 调度 Reaction → 队列稳定 → 处理可选终局/检查点 → `finishDraft` → 验证并生成候选 RuntimeSnapshot → 必要时持久化候选 StoredProfile → 一次性替换 Runtime 状态、baseline、revision 与 snapshot → 通知订阅者。

发布前任一步骤抛错时丢弃候选结果。持久化完成后的监控与订阅通知必须吞掉观察者自身异常，不能把已经提交的事务报告成回滚；不得尝试手工反向应用部分写入。

实现内部设置防失控常量：单处理单元最多 512 个 Action frame、4096 次 Rule 执行、128 次自动 CheckNode 跳转。超过限制统一返回 `script-error`；这些值是开发期防护参数，不是游戏包 API。
...
- 每次读取或调用都直接执行 Rule，不收集依赖、不缓存结果；
- 统计处理单元内的 Rule 总执行次数和耗时。

Config 只读且一局内不变，对 Config 的读取不登记依赖。Rule 不获得 random、action、endRun、容器元数据或真实时间。

### 9.5 Reaction

Reaction 在 Runtime 建立时只计算 baseline，不执行 Action。root 操作后以及每个 Reaction Action 后，引擎按 canonical 顺序重新收集并计算全部当前 watch，使用 `Object.is` 比较前后 Primitive，再应用 from/to。新出现的 Reaction 只建立 baseline；离开作用域的 Reaction 删除 baseline；已入队但执行前已经离开作用域的 Reaction 跳过。
...
- Session 结束时打印本局汇总：运行时长、command 数、Action 数、Rule 执行数、处理单元累计墙钟耗时、累计持久化耗时和最慢的五项记录；统计在 trace 到达时在线累计，最慢项固定保留五条，verbose 明细使用最多 200 条的 ring buffer；嵌套 Action 的 inclusive duration 不重复累加到总耗时；
...
当前自动用例覆盖 Config 感知坏档、Rule 返回契约、Reaction canonical 顺序与循环上限、eager Reaction 与 required blocker、selector/持久化失败回滚、`advance-turn` 部分提交、branch/truncate 独立性、历史检查点生命周期投影、确定性 PRNG、嵌套脚本调用链、构造失败时的 monitor 回收、IndexedDB 开发期清理、CAS 冲突、坏记录隔离和 monitor 因果链。测试使用纯内存 Repository 与 `fake-indexeddb`，不依赖现有游戏包，也不包含 UI 测试。
