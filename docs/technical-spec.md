...
5. 退出后继续、查看存档、创建分支、截断和 pin 检查点；
6. 处理放弃、终局、通用结果页和 restart；
7. 使用符合 `DESIGN.md` 的 React UI 完成玩家流程。

本轮不扩展[设计一致性审计](./game-design/design-audit.md#已确认但延期的边界)中的延期能力：不设计内容迁移模块、难度参数、通用媒体字段、稳定的外部 SaveRepository API 或完整调试工具协议。实现内部可以建立满足 MVP 的最小服务，但不把它们承诺为游戏包公共 API。
...
- 不引入动画库；过渡使用 CSS，并尊重 `prefers-smooth-motion`。
...
```mermaid
flowchart LR
    UI[React pages/components] --> Session[GameSession / app controllers]
    Session --> Runtime[GameplayRuntime]
    Session --> Saves[SaveRepository]
    Session --> Packages[GamePackageService]
...
- UI 只依赖 `SessionView`、存档页面 read model 和 controller 方法；不读取 Runtime Proxy。
...
所有错误转换成 `PackageLoadError`，保留 stage、包 id/version、资源路径和原始 cause。HTTP 非 2xx、JSON 语法错误、schema 错误和 linking 错误必须可区分。
...
- ReactiveValue、Reaction、Choice、Command、Check 引用的 Rule/Action key 已注册；
...
- schema 使用 `.strict()` 拒绝未知字段，版本化容器除外；
...
存档加载也必须先验证结构和领域约束，再构造 Runtime。不得把 IndexedDB 中的值直接断言成 `Profile`。

## 8. IndexedDB 持久化

### 8.1 数据库结构

数据库名：`maker-simulator`，MVP database version：`1`。

| Object store | key | index | 内容 |
| --- | --- | --- | --- |
| `profiles` | `profileId` | `by-config-id`、`by-updated-at` | 完整 `Profile` |
...
MVP 每次稳定保存直接写入完整 Profile。这样可以让 Profile、RunData、TurnData、恢复游标和 pin 修改位于同一个 IndexedDB record/transaction 中，先保证原子语义；增量快照和拆表优化不在本轮范围。
...
`put` 必须等待 `tx.done` 后才算成功。运行时 draft、Proxy、LoadedGamePackage 和函数不得传入 repository。写入前以 `structuredClone` 产生纯数据副本并进行 schema/领域校验。

不得在已开启的 IndexedDB transaction 中等待 fetch、脚本执行或 UI Promise。先在内存中完成整个新 Profile，再开启短事务一次写入。

### 8.3 保存边界

- 新游戏：带 `initial` 的 Profile 构造和 baseline 成功后首次保存；
...
- 对字面 ReactiveValue 允许 State 覆盖；
- 对带 Rule 的 ReactiveValue 始终返回 Rule 计算值，并拒绝同字段写入；
...
Rule Proxy、Action Proxy 和 UI selector 共用路径解析器，但权限不同。不要维护三份独立的合并逻辑。
...
Action Proxy 把不同 context scope 的写入路由到对应 draft。嵌套 Action、Reaction Action、EventInstance 派生写入和 PRNG 使用同一 draft。

提交顺序：执行 root 操作 → 验证 Action frame → 生成事件派生写入 → 失效 Rule → 调度 Reaction → 队列稳定 → 验证完整 draft → 处理可选终局 → `finishDraft` → 更新工作状态/检查点 → 必要时持久化 → 发布 snapshot。

任一步骤抛错时丢弃 draft。不得尝试手工反向应用部分写入。

实现内部设置防失控常量：单处理单元最多 512 个 Action frame、4096 次 Rule 重算、128 次自动 CheckNode 跳转。超过限制统一返回 `script-error`；这些值是 MVP 防护参数，不是游戏包 API。
...
- 执行期间记录所有 State Proxy `get`、ownKeys 和数组迭代依赖；
- 条件分支重算后替换旧依赖集合；
- 检测同步递归调用栈中的循环；
- 验证 ReactiveValue 和 Reaction watch 所需的返回类型；
- 不缓存异常结果。

Config 只读且一局内不变，对 Config 的读取不登记依赖。Rule 不获得 random、action、endRun、容器元数据或真实时间。

### 9.5 Reaction

Reaction 注册时构造独立 watch node，只计算 baseline，不执行 Action。写入后使用 `Object.is` 比较前后 Primitive，再应用 from/to。

注册范围和注销时机按设计文档执行。ordinal 使用已定义 canonical key，匹配项按 ordinal 进入 FIFO；新匹配追加队尾。首次 `endRun()` 只记录 pending 请求和可选来源 EventInstance，队列仍运行到稳定；后续请求不替换首次来源。

### 9.6 确定性随机数

MVP 使用项目内固定算法 `xmur3-mulberry32-v1`：
...
算法名称作为 engine stateVersion 1 的实现常量，不写入游戏包。更换算法必须提升 State schema 版本。游戏脚本不得使用 `Math.random()`。
...
```text
[maker-runtime] run=run-01 turn=3 unit=u-18 command choose-single 4.27ms ok
[maker-runtime] run=run-01 turn=3 unit=u-18   action event.choose-path 0.61ms ok
[maker-runtime] run=run-01 turn=3 unit=u-18   reaction effect.darkness 0.24ms ok
[maker-runtime] run=run-01 turn=3 unit=u-18 transaction commit 4.03ms ok
```

实现要求：

- 使用 `performance.now()` 计算单调高精度耗时，展示时保留两位小数；
- command 耗时从该项离开等待队列、开始执行时计时，到结果返回为止；包含其 Action/Reaction 和必要持久化，不包含之前命令造成的排队等待；
- Action 耗时包含该调用帧中的嵌套 Action；depth 用于表现调用层级；
- Reaction 日志使用 Reaction canonical key 作为 name，并在 detail 中记录 Action key；
- error/rollback 日志记录错误 code，不打印完整 State、Config 或存档内容；
- 处理单元结束后打印 Rule 汇总；verbose 模式才能打印单个 Rule；
- Session 结束时打印本局汇总：运行时长、command 数、Action 数、Rule 重算数、处理单元累计墙钟耗时、累计持久化耗时和最慢的五项记录；嵌套 Action 的 inclusive duration 不重复累加到总耗时；
...
使用 React Context 只做依赖注入，不把频繁变化的 runtime snapshot 放入 Context。
...
- 存档浏览器：白色主体、hairline 时间线；branch/restart 使用不同线型和 pastel 标记。危险截断使用 Dialog，不把整页染红。
...
- 完成依赖图、ReactiveValue、Reaction baseline/FIFO；
- 完成随机回滚、循环保护和结构化 script error；
- 完成 ConsoleRuntimeMonitor、NoopRuntimeMonitor 和逐处理单元 Rule 汇总。
...
- 完成 preview、continue、branch、truncate、pin；
...
最终由人工在开发服务器和 production preview 各验收一次：
