...
| `/arts/retrofuturism-pool` | 固定机位的 Three.js 复古未来泳池场景 |
...
路由页面使用 `React.lazy()` 按页面拆分；Arts Gallery 列表和项目详情位于游戏路由布局之外，列表只加载预先采集的场景渲染封面，Three.js 场景与后处理代码只在访问 `/arts/retrofuturism-pool` 时加载。Arts 列表与常规游戏页面共享 `PageChrome` 顶栏，并可通过主导航在 `/games` 与 `/arts` 之间切换；顶栏横跨视口并使用恒定的 sticky 与透明背景模糊样式。`GameLayout` 只在游戏路由匹配时加载，通过 `AppServicesProvider` 创建应用层组合根并用 `Outlet` 承载当前游戏页面，因此 Arts 页面不会加载包加载器、IndexedDB Repository、Runtime 或 Session，游戏页面也不会加载 Arts 场景资源。列表、菜单、存档和结果页只调用查询方法取得各自的 read model；游玩页只持有 `GameSession` 接口。页面不自行创建数据库连接、加载游戏包或取得具体 Runtime。
